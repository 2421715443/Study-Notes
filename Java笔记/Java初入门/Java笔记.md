...
向上转型就是多态的一种特性，在这里不在过多赘述，但我们知道导出类向上转型时，编译器是如何知道这个基类的引用参数到底是指向它的哪个导出类呢？

#### 后期绑定（也叫动态绑定或运行时绑定）

方法调用的绑定方式，若在程序执行前绑定就叫做前期绑定，他是面向过程的语言中默认的绑定方式，例如C语言就只有一种方法调用，那就是前期绑定。而向上转型就是用后期绑定（也叫动态绑定或运行时绑定）来解决指向问题的，其实Java中除了static和final(包括private)方法，其他所有的方法都是后期绑定，我们不必判断它是否会发生，他会自动发生，后期绑定时在对象中安置了某种“类型信息”，我们需要注意，final可以有效的“关闭”后期绑定从而调用生成更有效的代码，但大多数情况下并不会对性能有什么改观，所以不应该是为了追求性能而设计使用final，而是根据需求。
...
看了上面的代码你可能会以为s.draw会调用基类中的方法，但动态绑定却帮你正确的调用了导出类Circle中的方法。
